...
void PriorityQueue::PopNextN()
{
    ...
    // -------------------------------------------------------
    // Step 1: 进程0从队列头部取出 batch_size 个PT
    // batch_size 取 min(nprocs, priority.size())
    // -------------------------------------------------------
    int batch_size = 0;
    vector<PT> batch_pts;
    ...
    // -------------------------------------------------------
    // Step 2: 进程0将第i个PT的信息广播给进程i
    // 每个进程只处理自己编号对应的PT
    // 广播内容：prefix字符串 + 最后一个segment的type/len/total
    // -------------------------------------------------------
    ...
    vector<char> recv_buf(total_len > 0 ? total_len : 1);
    ...
    if (rank == 0)
    {
        string all_str(recv_buf.begin(), recv_buf.begin() + total_len);
        size_t pos = 0, found;
    ...
    // -------------------------------------------------------
    // Step 4: 进程0处理新PT生成并插回优先队列
    // 等所有PT生成完毕后，统一将新PT插回队列
    // -------------------------------------------------------
    if (rank == 0)
    {
        // 收集所有batch PT产生的新PT
