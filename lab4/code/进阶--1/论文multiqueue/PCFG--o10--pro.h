...
// 优先队列，用于按照概率降序生成口令猜测
// MultiQueue 版本：多个局部优先队列，每个局部队列内部用最大堆维护
class PriorityQueue
{
public:
    // 原来的单全局 priority 可以保留，但 MultiQueue 版本不再使用
    vector<PT> priority;
    ...
    // MultiQueue 参数
    int queue_factor = 4;   // 对应论文中的 c
    int num_queues = 0;     // num_queues = queue_factor * omp_get_max_threads()

    // 多个局部队列，每个 local_queues[i] 都是一个最大堆
    vector<vector<PT>> local_queues;

    // 每个局部队列一把锁
    vector<omp_lock_t> queue_locks;

    // 每个局部队列堆顶概率，方便随机选两个队列时快速比较
    vector<float> queue_top_probs;

    // 保护 guesses 和 total_guesses
    omp_lock_t guess_lock;

    bool multiqueue_inited = false;
    bool guess_lock_inited = false;
    ...
    // MultiQueue 辅助函数
    int RandomQueueId() const;
    void RefreshTopProb(int qid);
    void PushPT(PT &&pt);
    bool TryPopBestOfTwo(PT &out);
