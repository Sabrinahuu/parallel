...
void model::train(string path, int rank)
{
    string pw;
    ifstream train_set(path);
    int lines = 0;
    if (rank == 0) cout<<"Training..."<<endl;
if (rank == 0) cout<<"Training phase 1: reading and parsing passwords..."<<endl;
    while (train_set >> pw)
    {
        lines += 1;
        if (lines % 10000 == 0)
...
void segment::order(int rank)
{
    ...
    // 下面这个重复的for循环必须删掉
    // for (string val : ordered_values) { ... }  ← 删掉这个
}
...
{
    if (rank == 0)cout << "Training phase 2: Ordering segment values and PTs..." << endl;
    ...
    bool swapped;
    if (rank == 0)cout << "total pts" << ordered_pts.size() << endl;
    std::sort(ordered_pts.begin(), ordered_pts.end(), compareByPretermProb);
    if (rank == 0)cout << "Ordering letters" << endl;
    // cout << "total letters" << endl;
    for (int i = 0; i < letters.size(); i += 1)
    {
        // cout << i << endl;
        letters[i].order(rank);
    }
    if (rank == 0)cout << "Ordering digits" << endl;
    // cout << "total letters" << endl;
    for (int i = 0; i < digits.size(); i += 1)
    {
        digits[i].order(rank);
    }
    if (rank == 0)cout << "ordering symbols" << endl;
    // cout << "total letters" << endl;
    for (int i = 0; i < symbols.size(); i += 1)
    {
        symbols[i].order(rank);
    }
}