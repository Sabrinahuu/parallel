...
// ── 自适应调度阈值 ──
static const int GPU_SINGLE_THRESHOLD = 1000;   // 单个 PT 的 value 数少于此值，走 CPU
static const int BATCH_THRESHOLD      = 200000; // 批次攒够此数量，提交 GPU
...
void PriorityQueue::Generate(PT pt)
{
    ...
    int n = (int)a->ordered_values.size();
 
    // ── 自适应调度：小任务直接 CPU，大任务进 GPU 批次 ──
    if (n < GPU_SINGLE_THRESHOLD)
    {
        // CPU 直接生成，零传输开销
        for (int i = 0; i < n; i++)
