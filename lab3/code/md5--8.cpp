...
{
    // 短消息固定生成 1 个 64B block，直接清零整个 block
    uint64_t *q = reinterpret_cast<uint64_t *>(out);
    q[0] = 0;
    q[1] = 0;
    q[2] = 0;
    q[3] = 0;
    ...
    // 8 字节粒度复制，避免调用通用 memcpy@plt
    for (; i + 8 <= length; i += 8)
    {
        uint64_t v;
        __builtin_memcpy(&v, data + i, 8);
        __builtin_memcpy(out + i, &v, 8);
    }
    ...
    if (i + 4 <= length)
    {
        ...
        __builtin_memcpy(out + i, &v, 4);
        i += 4;
    }

    // 剩余 0~3 字节
    for (; i < length; ++i)
        out[i] = static_cast<Byte>(data[i]);

    out[length] = 0x80;

    uint64_t bitLen = static_cast<uint64_t>(length) * 8;
    __builtin_memcpy(out + 56, &bitLen, 8);
...
#define LOAD_X(name, k) \
    uint32x4_t name = { w0[k], w1[k], w2[k], w3[k] }
