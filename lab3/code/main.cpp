...
int main()
{
    ...
    while (!q.priority.empty())
    {
        ...
        if (curr_num > 1000000)
        {
            ...
            //
            // 现在替换为批量SIMD哈希。
            // q.guesses是vector<string>，MD5HashBatch_NEON内部每4个字符串一组进行NEON并行计算。
            // 当前主程序只统计Hash time，不输出每个hash，所以第二个参数传nullptr。
            MD5HashBatch_NEON(q.guesses, nullptr);

            // 如果你之后需要输出hash，可以改成：
            //
            // vector<array<bit32, 4>> hash_results;
            // MD5HashBatch_NEON(q.guesses, &hash_results);
            //
            // 然后再遍历hash_results输出。
